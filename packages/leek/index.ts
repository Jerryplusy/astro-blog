...
export default function AstroLeekIntegration(opts: UserInputConfig): AstroIntegration {
    ...
    hooks: {
      'astro:config:setup': async ({ config, updateConfig }) => {
        let userConfig = parseWithFriendlyErrors(
          ...
          UserConfigSchema,
          opts,
          'Invalid config passed to astro-leek integration'
        )
        ...
        const selfIndex = config.integrations.findIndex((i) => i.name === 'astro-leek')
